...
//旧流
	...
	if (p_info_hd->isin)
	{
			...
			if((telnet_info[p_num][num].log_state==2)&&(!memcmp(">",payload+p_info_hd->payload_len-1,1)))
			{
				...
				//	插入用户名密码
					...
					*/
					...
					sprintf(tml_file[p_num],"Sql_terminal_%lu_%d_%d",p_info_hd->audit_time,p_num,telnet_info[p_num][num].cmd_id);
					...
					sprintf(str_sql[p_num],"rowkey=%lu|colfam1:table=%s|colfam1:app_id=%u|colfam1:security_level=%hu|colfam1:src_ip=%s|colfam1:dst_ip=%s|colfam1:src_mac=%s|colfam1:dst_mac=%s|colfam1:src_port=%u|colfam1:dst_port=%u|colfam1:user_name=%s|colfam1:interval_time=%d|colfam1:operation_command=%s|colfam1:line_num=%d|colfam1:response_content=%d|colfam\n",p_info_hd->audit_time,tablesTime,3,p_info->security_level,src_ip,dst_ip,src_mac,des_mac,p_info_hd->cli_port,p_info_hd->ser_port,telnet_info[p_num][num].user,0,telnet_info[p_num][num].cmd,1,pass_len);
		...
		else
		{
			...
			if((telnet_info[p_num][num].res_len + p_info_hd->payload_len < (RES_MAX_LEN-2000) ) ||((telnet_info[p_num][num].res_len + p_info_hd->payload_len >(RES_MAX_LEN-1000))&&telnet_info[p_num][num].end_flag ==1))
			{
			...
			else
			{
				...
				if(strlen(telnet_info[p_num][num].cmd)>0)
				{
					...
					//数据库语句cmd
							...
							//printf("123\n");
							time(&telnet_info[p_num][num].end);
							duration=(int)difftime(telnet_info[p_num][num].end,telnet_info[p_num][num].start);
							...
							sprintf(str_sql[p_num],"rowkey=%lu|colfam1:table=%s|colfam1:app_id=%u|colfam1:security_level=%hu|colfam1:saveflag=%d|colfam1:src_ip=%s|colfam1:dst_ip=%s|colfam1:src_mac=%s|colfam1:dst_mac=%s|colfam1:src_port=%u|colfam1:dst_port=%u|colfam1:user_name=%s|colfam1:interval_time=%d|colfam1:operation_command=%s|colfam1:line_num=%d|colfam1:response_content=%d|colfam\n",p_info_hd->audit_time,tablesTime,3,p_info->security_level,save_flag,src_ip,dst_ip,src_mac,des_mac,p_info_hd->cli_port,p_info_hd->ser_port,telnet_info[p_num][num].user,duration,telnet_info[p_num][num].cmd,line_num,telnet_info[p_num][num].res_len);
	...
	else
	{
		...
		if(!telnet_info[p_num][num].cmd_state)   //未登录
		{
			...
			{	
				...
				{
					if(telnet_info[p_num][num].stack_num < STACK_MAX_NUM)
					{
					//	up down键处理
					...
					{
						if(stack_index == telnet_info[p_num][num].stack_num)//到底层按下键，返回命令为"",cmd_pos置位为顶层
						{
							telnet_info[p_num][num].cmd[0] ='\0';
							stack_index = 0;
						}
				...
				time(&telnet_info[p_num][num].start);
