...
//旧流
	...
	if (p_info_hd->isin)
	{
		...
		else
		{
			...
			if(telnet_info[p_num][num].res_len + p_info_hd->payload_len < (RES_MAX_LEN-1000)  ||((telnet_info[p_num][num].res_len + p_info_hd->payload_len >(RES_MAX_LEN-1000))&&telnet_info[p_num][num].end_flag ==1))
			{
	...
	else
	{
		...
		if(!telnet_info[p_num][num].cmd_state)   //未登录
		{
			...
			{	
				...
				if(strlen(telnet_info[p_num][num].cmd)<1 || insert_flag)
				{
					telnet_info[p_num][num].cmd_pos = telnet_info[p_num][num].stack_num;
					;//不写入缓存
				}
