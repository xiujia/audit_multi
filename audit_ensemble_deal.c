...
int check_cache_type(char * request_file){
			...
			if((fsize>28||fsize==28)&&*buf==0x00&&*(buf+1)==0x00&&*(buf+2)==0x00&&*(buf+3)==0x00){
				pos = buf+14;
				if(*(pos+12)==0x42 && *(pos+13)==0x49 ){
					return 1;
				}
...
int check_request_type(char * request_file){
	...
	//		printf("requesturl:%s\n",request_url);
			...
			if(strcasestr(request_url,"websys.csp") ==NULL && strcasestr(request_url,"websys.default.csp")==NULL){
				return -1;
			}
